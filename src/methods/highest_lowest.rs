...
/// let mut hld = HighestLowestDelta::new(3, values.get(0).unwrap()).unwrap();
///
/// (0..values.len()).for_each(|i| {
///     let v = hld.next(values.get(i).unwrap());
///     assert_eq!(v, r[i]);
/// });
/// ```
///
...
///
/// let mut highest = Highest::new(3, values.get(0).unwrap()).unwrap();
///
/// (0..values.len()).for_each(|i| {
///     let v = highest.next(values.get(i).unwrap());
///     assert_eq!(v, r[i]);
/// });
/// ```
///
...
///
/// let mut lowest = Lowest::new(3, values.get(0).unwrap()).unwrap();
///
/// (0..values.len()).for_each(|i| {
///     let v = lowest.next(values.get(i).unwrap());
///     assert_eq!(v, r[i]);
/// });
/// ```
///
