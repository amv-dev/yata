...
mod tests {
	...
	fn test_past1() {
		...
		candles.take(100).for_each(|x| {
			...
			approx::assert_abs_diff_eq!(p.close, q.close);
			approx::assert_abs_diff_eq!(p.volume, q.volume);
			approx::assert_abs_diff_eq!(p.high, q.high);
			prev = Some(x);
		});
	}
	...
	fn test_past() {
		...
		(1..20).for_each(|length| {
			...
			src.iter().enumerate().for_each(|(i, &x)| {
				approx::assert_abs_diff_eq!(src[i.saturating_sub(length as usize)], ma.next(x))
			});
		});
	}
}
